    ...
    <nav v-if="!loading && charactersPageMetadata" aria-label="Page navigation example">
      <ul class="pagination">
        <li v-if="page !== 1" class="page-item">
          <router-link class="page-link" :to="{ path: '/characters', query: { page: page - 1 } }">
            Prev
          </router-link>
        </li>
        <div v-for="pageNumber in charactersPageMetadata.totalPages" :key="`page-${pageNumber}`">
          <li v-if="pageNumber == page ||
              ...
              class="page-item"
          >
            <router-link class="page-link" :to="{ path: '/characters', query: { page: pageNumber } }">
              {{ pageNumber }}
            </router-link>
          </li>
        </div>
